...
| Purchases and library    | Paid papers use LeeTec STK Push, server-side transaction-history verification, idempotent fulfilment, entitlement records, and protected downloads.             | Preserve payment references, entitlement checks, purchase history, and audit events.                         |
...
| Wallet                   | Users initiate wallet top-ups through LeeTec STK Push, receive an asynchronous payment state, and see confirmed balance and ledger history.                     | Preserve server-side payment confirmation and idempotent ledger updates.                                     |
